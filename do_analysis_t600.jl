...
ds_out.attrib["title"] = "Statstics per ensemble member FOR ONLY THE CONVECTIVE TRAJECTORIES."
ds_out.attrib["description"] = "For physical variables the first dimension is ensemble member from 20 to 89, second dimension is statistics in this order: mean, median, std, 5th, 10th, 25th and 75th percentile. For ntrajs its just one entry per ensemble member"
...
#You CAN run this in multithread, but I can make no promises. If you want to,
#then change the commented for loop initialization below accordingly 
#and also look out for the locks in the function "process_array".
...
for i in 1:70
	...
	print("$(m), ")
        ...
        t6 = calc_t600(m)
        msk = t6 .< 10 #This is where the fastest trajs are selected
	...
	process_array(MaxnHm, MnH_out, i)
        ...
        tau = tau_rel(pmu,Tmu,qni_end,qi_end)
        ...
        for j in 1:length(p_gl99)
                strt = find_start(p[j,:])
                if nm.minimum(liq_frac[j,strt:end]) > 0.001
                        p_gl99[j] = NaN
                        T_gl99[j] = NaN
                        qfr99[j] = NaN
                        qi99[j] = NaN
                        ...
                        ind = findfirst(liq_frac[j,strt:end] .< 0.001) + strt - 1
                        p_gl99[j] = p[j,ind]
                        T_gl99[j] = T[j,ind]
                        qfr99[j] = qfr[j,ind]
                        qi99[j] = qi[j,ind]
